...
// Uso del proyecto principal
const project = new MainProject();

// Ejemplos de interacción:

// Cambiar altura de un cubo
// project.changeCubeHeight(0, 25);

// Añadir un nuevo cubo
// project.addCube(30, 30, 15);

// Cambiar paleta de colores
// project.changeColorPalette([0xFF0000, 0x00FF00, 0x0000FF, 0xFFFF00, 0xFF00FF, 0x00FFFF]);

// Cambiar nivel de detalle
// project.changeDetailLevel('medium');

// Obtener estadísticas
// console.log(project.getCityStats());

// Event listeners para UI (ejemplos)
document.addEventListener('keydown', (event) => {
    switch(event.key) {
        case '1':
            project.changeDetailLevel('low');
            break;
        case '2':
            project.changeDetailLevel('medium');
            break;
        case '3':
            project.changeDetailLevel('high');
            break;
        case 'r':
            // Regenerar con alturas aleatorias
            project.cubes.forEach((cube, index) => {
                const newHeight = Math.random() * 25 + 5;
                project.changeCubeHeight(index, newHeight);
            });
            break;
        case 'c':
            // Cambiar paleta de colores aleatoria
            const randomPalette = Array(6).fill().map(() => 
                Math.floor(Math.random() * 0xFFFFFF)
            );
            project.changeColorPalette(randomPalette);
            break;
    }
});
